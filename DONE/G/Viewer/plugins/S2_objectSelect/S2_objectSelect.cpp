...
void ObjectSelect::onPluginLoad() {
    ...
    vs->compileSourceFile("objectSelect.vert");

    fs = new QOpenGLShader(QOpenGLShader::Fragment);
    fs->compileSourceFile("objectSelect.frag");
...
void ObjectSelect::selectDraw(GLWidget & g) {
    ...
    for (unsigned int i=0; i<scene()->objects().size(); ++i){
        GLubyte color[4];
        encodeID(i,color);
        program->setUniformValue("color", QVector4D(color[0]/255., color[1]/255., color[2]/255., color[3]/255.));
