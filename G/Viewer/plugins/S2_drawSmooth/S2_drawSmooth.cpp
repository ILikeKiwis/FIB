...
void Draw_smooth::addVBO(unsigned int currentObject)
{
    ...
    if (useIndices)
    {
        ...
        for (unsigned int i = 0; i < verts.size(); i++)
        {
			...
			colors.push_back(Ns[i].x());
			colors.push_back(Ns[i].y());
			colors.push_back(Ns[i].z());
    ...
    else
    {
        ...
        for (auto&& f: obj.faces()) 
        {
			...
			QVector3D n_avg = QVector3D(0, 0, 0);

			for (unsigned int i = 0; i < f.numVertices(); i++) {
				const Vector N = Ns[f.vertexIndex(i)];
				n_avg += N; 
			}
            n_avg = n_avg / float(f.numVertices());
            auto pushVertex = [&](int vertexIdx, int normalIdx, int tcIdx)
            {
                ...
                normals.push_back(n_avg.x());
				normals.push_back(n_avg.y());
				normals.push_back(n_avg.z());
