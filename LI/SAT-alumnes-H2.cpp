...
unordered_map<int, pair<int, int>> apariciones;
...
multimap<pair<int, int>, uint, Cmp> apOrder;
...
void readClauses()
{
    ...
    for (uint i = 0; i < numClauses; ++i)
    {
        ...
        while (cin >> lit and lit != 0)
        {
            ...
            if (lit > 0) apariciones[lit].first++;
            else apariciones[-lit].second++;
        }
    }
...
{
    for (auto it = apOrder.begin(); it != apOrder.end(); ++it) // stupid heuristic:
        if (model[it->second] == UNDEF)
            return it->second; // returns first UNDEF var, positively
