...
%% village.  These towers can be used to watch horizontally and vertically (like the rooks
...
% YOU MAY WANT TO INTRODUCE SOME OTHER VARIABLE FOR MAKING THE CARDINALITY CONSTRAINTS SMALLERR
...
writeClauses(MaxNumTowers):-
    eachVillageAtLeastWatchedByOneTower, 
    significantVillagesExactlyOneTower, %% atLeast
    atMostOneTowerPerVillage,   
    validPosForTowers,         
    respectMaxNumTowers(MaxNumTowers),
...
respectMaxNumTowers(MaxNumTowers) :-
        findall(towerPos(I, J), posVillage(_, I, J), Lits), 
        atMost(MaxNumTowers, Lits),
        fail.
respectMaxNumTowers(_).
...
costOfThisSolution(M,Cost):- 
        findall((I,J), member(towerPos(I, J), M), Towers), 
        sort(Towers, T), 
        length(T, Cost),!.
