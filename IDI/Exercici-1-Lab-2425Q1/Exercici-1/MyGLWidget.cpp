...
void MyGLWidget::pintaParpella(){
  ...
  modelTransformQuadrat(glm::vec3(-0.15, 0.25, 0.), glm::vec3(0.2, ty, 0.2));
  glDrawArrays(GL_TRIANGLES, 0, 6);
}
...
void MyGLWidget::pintaCap(){
  // Pintem el cap
  glBindVertexArray(VAO_Quadrat_verd);
  modelTransformQuadrat(glm::vec3(0., 0.4, 0.), glm::vec3(0.8));
  glDrawArrays(GL_TRIANGLES, 0, 6);    	
}	
...
void MyGLWidget::modelTransformMandibula(bool up, double angleApertura)
{
  glm::mat4 TG(1.0f);
  TG = glm::translate(TG, glm::vec3(tx, 0, 0));
  if (!up) TG = glm::translate(TG, glm::vec3(0.5*0.6, -0.2*0.6, 0));      //multimpliquem per 0.6 per el scale, i 0.5 es la meitat del w de la mandibula i 0.2 la meitat de la h total de la mandibula per mourels desde el 0,0
  else TG = glm::translate(TG, glm::vec3(0.6*(0.5-(1./8.)),0 ,0));
  TG = glm::scale(TG, glm::vec3(0.6));                            
  if (up) {
    TG = glm::translate(TG, glm::vec3(-0.5, 0, 0));
    TG = glm::rotate(TG, float(glm::radians(angleApertura)), glm::vec3(0, 0, 1));
    TG = glm::translate(TG, glm::vec3(0.5, 0, 0));
    TG = glm::rotate(TG, float(glm::radians(180.)), glm::vec3(0, 0, 1.0));
  }
  TG = glm::translate(TG, glm::vec3(-0.5, -0.2, 0.));
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::creaBuffersMandibula()
{
  ...
  for(int i=0;i<numVertexs;i++) {
      if (i < 6) Colors[i] = verd;
      else {
        if (i % 3 == 1) Colors[i] = blanc_dent;
        else Colors[i] = blanc;
      }
  }
